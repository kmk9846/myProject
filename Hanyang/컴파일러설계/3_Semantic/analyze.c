...
{ TreeNode * func;
  ...
  param = newParamNode(NonArrayParameterK);   // single integer parameter
  param->attr.name = "arg";
  param->type = Integer;
    ...
    case ExpK:
      ...
      { case AssignK:
        ...
        { BucketList l = get_bucket(t->attr.name);
          ...
          { if ((symbolNode->nodekind == DeclareK && symbolNode->kind.decl != ArrayVariableK)
                || (symbolNode->nodekind == ParameterK && symbolNode->kind.param != ArrayParameterK))
        ...
        { BucketList l = get_bucket(t->attr.name);
          ...
          if (funcNode->kind.decl != FunctionK)
          { typeError(t, "invalid expression");
            break;
          }
