...
def display():
    ...
    time = -0.5*np.cos(animTime * np.pi ) + 0.5
    R3=R1*Rotation.from_rotvec(time *delta)

    
    # drawCow(cow2wld1);											# Draw cow.
    # drawCow(cow2wld2);											# Draw cow.
    drawCow(matrix4(R3.as_matrix(), getTranslation(cow2wld1 + time * (cow2wld2 - cow2wld1))))
...
def main():
    ...
    width = 800
    height = 600
    window = glfw.create_window(width, height, '2018007938-7-1', None, None)
    if not window:
        glfw.terminate()
        sys.exit(-1)
