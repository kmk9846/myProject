...
def render():
    ...
    glLoadIdentity()
    gluLookAt(5*np.sin(gCamAng),gCamHeight-0.1,5*np.cos(gCamAng), 0,0,0, 0,1,0)
    ...
    Ry = np.array([[np.cos(Alpha), -np.sin(Alpha), 0],
...
def main():
    ...
    if not glfw.init():
        return
    window = glfw.create_window(480,480,'2018007938-6-1', None,None)
    if not window:
        glfw.terminate()
        return
